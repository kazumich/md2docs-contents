...
実は **必須キーは無し**。`title` を省略すれば本文先頭の `# 見出し` を自動的にタイトル扱いし、`date` を省略すればファイルの mtime をフォールバックとして使います。最小構成では Markdown 本文だけで書き始められます。

## よく使うキー

| キー | 型 | 説明 |
|------|-----|------|
| `title` | string | ページタイトル。省略時は本文先頭の `# 見出し` を抽出（その行は本文から除去） |
| `date` | string | `Y-m-d H:i:s` 形式の公開日時。省略時は ファイル mtime |
| `description` | string | 一覧・OGP の説明文 |
| `order` | int | セクション内の並び順（小さいほど上） |
| `tags` | string[] | タグ配列 |
| `author` | string | 著者名 |
| `category` | string | セクションスラッグ（ファイル配置で決まるため省略可） |
| `status` | `"open"` / `"close"` | 公開ステータス。`close` で一覧・ナビ・検索から除外（URL 直リンクは可）。省略時は `open` |
